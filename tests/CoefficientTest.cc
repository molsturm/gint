...
using namespace sturmint::orbital_index;
  ...
  MultiVector<SmallVector<real_type> > multivector_from_pointer(const real_type *data, size_t n_rows,
							       size_t n_vectors)
  {
    MultiVector<SmallVector<real_type> > mv(n_rows,n_vectors);

    for(size_t i=0, ij=0;i<n_vectors;i++)
      for(size_t j=0;j<n_rows;j++,ij++)
	mv[i][j] = data[ij];
  ...
  MultiVector<SmallVector<real_type> > Coefs(multivector_from_pointer(&Coefs_vector[0],n_orbitals,n_electrons));
...
SECTION("Test J") {
  auto Jbbconv = static_cast<stored_matrix_type>(J_bb);
    const NumCompAccuracyLevel apply_tol = NumCompAccuracyLevel::Default;
      CHECK(rc::check("Test J",
                      make_apply_ptr_vector_test(J_bb, Jbbconv, apply_tol)));
}

SECTION("Test K") {
  auto Kbbconv = static_cast<stored_matrix_type>(K_bb);
    const NumCompAccuracyLevel apply_tol = NumCompAccuracyLevel::Default;
      CHECK(rc::check("Test K",
                      make_apply_ptr_vector_test(K_bb, Kbbconv, apply_tol)));
}
